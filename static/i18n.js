window.LOCALES = {
  ...
  'zh-CN': {
    ...
    UI: {
      ...
      webhookTemplateLabel:  '请求模板 <span style="color:var(--text-faint);font-weight:400;">（首行 <code>METHOD URL</code>，其下为请求头，之后即为请求体；占位符 ##FROM## ##TO## ##CONTENT##，写进 JSON 时改用 ##CONTENT_IN_JSON## 等以自动转义引号/换行）</span>',
      webhookDocsText:       '📖 查看完整格式说明 ↗',
      webhookDocsLink:       'https://github.com/Dr-Mos/sms-gateway/blob/master/docs/webhook-template.zh-CN.md',
  ...
  'zh-TW': {
    ...
    UI: {
      ...
      webhookNameLabel:      '名稱',
      webhookNamePlaceholder:'例如：Bark 通知',
      webhookTemplateLabel:  '請求範本 <span style="color:var(--text-faint);font-weight:400;">（首行 <code>METHOD URL</code>，其下為請求標頭，之後即為請求主體；佔位符 ##FROM## ##TO## ##CONTENT##，寫進 JSON 時改用 ##CONTENT_IN_JSON## 等以自動跳脫引號/換行）</span>',
      webhookDocsText:       '📖 查看完整格式說明 ↗',
      webhookDocsLink:       'https://github.com/Dr-Mos/sms-gateway/blob/master/docs/webhook-template.zh-CN.md',
  ...
  'en': {
    ...
    UI: {
      ...
      webhookTemplateLabel:  'Request Template <span style="color:var(--text-faint);font-weight:400;">(first line <code>METHOD URL</code>, then headers, then the body; placeholders ##FROM## ##TO## ##CONTENT##, use ##CONTENT_IN_JSON## etc. inside JSON to auto-escape quotes/newlines)</span>',
      webhookDocsText:       '📖 Full format guide ↗',
      webhookDocsLink:       'https://github.com/Dr-Mos/sms-gateway/blob/master/docs/webhook-template.md',
  ...
  'ja': {
    ...
    UI: {
      ...
      webhookTemplateLabel:  'リクエストテンプレート <span style="color:var(--text-faint);font-weight:400;">（1行目 <code>METHOD URL</code>、続いてヘッダー、その後がボディ；プレースホルダー ##FROM## ##TO## ##CONTENT##、JSON 内では ##CONTENT_IN_JSON## 等で引用符/改行を自動エスケープ）</span>',
      webhookDocsText:       '📖 フォーマット詳細 ↗',
      webhookDocsLink:       'https://github.com/Dr-Mos/sms-gateway/blob/master/docs/webhook-template.md',
